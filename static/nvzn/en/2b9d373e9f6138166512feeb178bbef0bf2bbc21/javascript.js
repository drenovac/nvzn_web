Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-31-gc8e4b55-110",isDemo:YES,canEditManager:NO,canApproveManager:NO,showInlineSite:true,canApproveManagerDidChange:function(){if(!this.get("canApproveManager")){this.set("canEditManager",NO)
}}.observes("canApproveManager"),showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create({hour:0,minute:0,second:0}),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")
...
})}.property("timeCards","weekEnding").cacheable(),payslips:function(){return[{date:SC.DateTime.create(),paid_to:SC.DateTime.create().adjust({days:5}),hours:38,taxable:608,non_tax:0,gross:0,tax:-57,net:557,url:"/payslips/key1/slip.pdf"},{date:SC.DateTime.create().adjust({days:7}),paid_to:SC.DateTime.create().adjust({days:12}),hours:38,taxable:608,non_tax:0,gross:0,tax:-57,net:557,url:"/payslips/key2/slip.pdf"}].map(function(hash){return SC.Object.create(hash)
})}.property().cacheable()});Nvzn.employeesController=SC.ArrayController.create({contentBinding:"Nvzn.customerController.employees",allowsEmptySelection:NO,orderBy:["customer","lastName"],arrangedObjects:function(){var customerId=null,customer,employees=this.get("content"),site,storeKey,S=Nvzn.store,C=Nvzn.Customer,ret=[];
...
}.property("firstName","lastName").cacheable(),displayName:function(){return"%@, %@".fmt(this.get("lastName"),this.get("firstName")).titleize()
}.property("firstName","lastName"),timeCardsByDate:function(sunday){if(!sunday){sunday=Nvzn.get("weekEnding").adjust({hour:23,minute:59,second:59})
...
}var ret="",classes="timecard-cell ",allClasses,status,approveClass="approve",cid,approveClasses,sent=Nvzn.local.getPath("sent"),desc;
if(Nvzn.canEditManager){classes="editable-cell "+classes}var color=Nvzn.get("showTimeCardColors");
items.forEach(function(item){status=item.get("status");approveClasses=approveClass;
if(status&SC.Record.DIRTY){approveClasses+=" changed"}if(sent&&sent[item.get("id")]){approveClasses+=" submitted"
}if(item.get("approved")){approveClasses+=" approved"}if(Nvzn.canApproveManager){ret+="<span class='"+approveClasses+"' storeKey='"+item.get("storeKey")+"'> </span>"
}cid=item.getPath("customer");if(color){ret+="<span class='customer-color' title='"+cid+"' style='background-color:"+Nvzn.colorFor(cid)+";'>"
...
if(Nvzn.showInlineSite){ret+="<span class='site-name'>"+cid+"</span><br>"}desc=item.get("desc");
if(item.get("type")!=="N"&&desc!==null){ret+="<span class='tc_desc' title='%@'>%@</span></br>".fmt(desc,desc)
...
Nvzn.APP=Ki.State.design({initialSubstate:"ROSTER",enterState:function(){Nvzn.getPath("mainPage.mainPane").append();
if(Nvzn.loginController.get("role")=="site"){Nvzn.getPath("mainPage.tabView").set("items",[{title:"View Rosters",value:"all_employees"}])
}else{Nvzn.getPath("mainPage.tabView").set("items",[{title:"Roster",value:"sites_by_week"},{title:"Payslip",value:"payslips"}])
}},tabDidChange:function(){var nowShowing=Nvzn.mainPage.getPath("tabView.nowShowing");
switch(nowShowing){case"all_employees":this.gotoState("APP.ROSTER");break;case"approve_tc":this.gotoState("APP.ROSTER");
break;case"all_sites":this.gotoState("APP.ROSTER");break;case"sites_by_week":this.gotoState("APP.ROSTER");
break;case"ohs_report":this.gotoState("OHS_REPORT");break;case"payslips":this.gotoState("PAYSLIPS");
break;default:console.log("Tab Changed with no action:",nowShowing)}return YES},logout:function(){var url="/api/v1.1/logout";
SC.Request.getUrl(url).notify(this,function(){Nvzn.statechart.sendEvent("didLogout")
}).json().send()},didLogout:function(){window.location.reload()},ROSTER:Ki.State.plugin("Nvzn.ROSTER"),PAYSLIPS:Ki.State.plugin("Nvzn.PAYSLIPS"),OHS_REPORT:Ki.State.plugin("Nvzn.OHS_REPORT")});
...
Nvzn.formController.set("content",form)}})});Nvzn.PAYSLIPS=Ki.State.design({enterState:function(){Nvzn.getPath("mainPage.header.titleView").set("value","Payslips")
},exitState:function(){console.log("Payslips Exit");Nvzn.getPath("mainPage.header.titleView").set("value","")
}});Nvzn.ROSTER=Ki.State.design({initialSubstate:"START",enterState:function(){var controller=Nvzn.loginController,role=controller.get("role");
window.setTimeout(function(){Nvzn.mainPage.mainPane.get("calendar").resetToSelectedDate()
...
},exitState:function(){Nvzn.set("mode","none")},LOADING_EMPLOYEE:Ki.State.design({enterState:function(){Nvzn.rosterController.set("loading",YES);
...
Nvzn.getPath("mainPage.header").set("isVisible",YES)},exitState:function(){Nvzn.set("mode","none")
...
controller.set("content",Nvzn.store.materializeRecord(userKey));if(controller.get("db_id")){Nvzn.local.set("userDomain",controller.get("db_id"))
...
Nvzn.CHECK_LOGIN=Ki.State.design({enterState:function(){if($.browser.msie&&$.browser.version<9){var pane=Nvzn.getPath("loginPage.chromeFramePane");
pane.append();CFInstall.check({mode:"overlay",node:"prompt"});return}SC.Request.getUrl("/api/v1.1/login").json().notify(this,"checkedLogin").send()
...
},didLogin:function(req){var controller=Nvzn.loginController,body;controller.set("errorMessage","Logging in!");
body=req.get("body");if(req.status===401){controller.set("errorMessage","Incorrect Login or Password");
...
}}});Nvzn.statechart=Ki.Statechart.create({trace:YES,rootState:Ki.State.design({initialSubstate:"LOGIN",LOGIN:Ki.State.plugin("Nvzn.LOGIN"),LOADING:Ki.State.plugin("Nvzn.LOADING"),APP:Ki.State.plugin("Nvzn.APP")})});
...
Nvzn.loginPage=SC.Page.design({chromeFramePane:SC.Pane.design({layout:{top:0,bottom:0,right:0,left:0},childViews:"header prompt".w(),header:SC.LabelView.design({layout:{top:20,width:800,centerX:0,height:50},value:"<h2>Envizion was made for use with a modern browser.</h2><p>Please upgrade to the newest Internet Explorer, or install Chrome Frame. Chrome Frame does not need Admin rights.</p>",textAlign:SC.ALIGN_MIDDLE,escapeHTML:NO}),prompt:SC.View.design({layerId:"prompt",layout:{top:50,left:0,right:0,bottom:0}})}),loginPane:SC.Pane.design({layout:{centerX:0,centerY:0,width:450,height:330},childViews:("logo errorMessage loginLabel loginInput passLabel passInput submitButton demoLabel approveCheck editCheck employeeSiteCheck").w(),classNames:"login",defaultResponder:"Nvzn.statechart",focusFrom:function(pane){console.log("loginPane FocusFrom",pane)
},logo:SC.ImageView.design({layout:{width:200,right:100,top:0,height:70},value:"/static/nvzn/en/2b9d373e9f6138166512feeb178bbef0bf2bbc21/source/resources/image/edmen-logo-small.png"}),prompt:SC.LabelView.design({layout:{top:75,right:100},textAlign:SC.ALIGN_RIGHT,value:"Enter your user name and password below to login.",classNames:"black"}),errorMessage:SC.LabelView.design({layout:{top:189,left:10,height:20,width:300},classNames:["error-message"],textAlign:SC.ALIGN_RIGHT,valueBinding:"Nvzn.loginController.errorMessage"}),loginLabel:SC.LabelView.design({layout:{top:120,left:40,width:95,height:20},escapeHTML:NO,textAlign:SC.ALIGN_RIGHT,value:"<h1>Login</h1>",classNames:"black"}),loginInput:SC.TextFieldView.design({layout:{left:150,top:116,width:200,height:25},applyImmediately:YES,hint:"jsmith",valueBinding:"Nvzn.loginController.loginInput"}),passLabel:SC.LabelView.design({layout:{left:40,top:160,width:95,height:20},escapeHTML:NO,textAlign:SC.ALIGN_RIGHT,value:"<h1>Password</h1>",classNames:"black"}),passInput:SC.TextFieldView.design({layout:{left:150,top:156,width:200,height:25},applyImmediately:YES,valueBinding:"Nvzn.loginController.passInput",isPassword:YES,hint:"*******"}),forgotPassLink:SC.LabelView.design({layout:{top:185,left:150},escapeHTML:NO,value:'<a style="color: black" href="#">Forgotten your login or password?</a>'}),submitButton:SC.ButtonView.design({layout:{left:250,top:220,width:100,height:24},isDefault:YES,supportFocusRing:YES,title:"Login",action:"submitLogin"}),cancelButton:SC.ButtonView.design({layout:{left:90,top:220,width:100,height:24},title:"Cancel"}),demoLabel:SC.LabelView.design({layout:{top:250,left:40,width:400,height:20},isVisibleBinding:SC.Binding.oneWay("Nvzn.isDemo"),escapeHTML:NO,textAlign:SC.ALIGN_LEFT,value:"In Demo Mode you can enable the following options.",classNames:"black"}),approveCheck:SC.CheckboxView.design({layout:{top:270,left:40,width:300,height:20},isVisibleBinding:SC.Binding.oneWay("Nvzn.isDemo"),title:"Manager Login: Approve Time Cards",valueBinding:"Nvzn.canApproveManager"}),editCheck:SC.CheckboxView.design({layout:{top:290,left:40,width:300,height:20},isVisibleBinding:SC.Binding.oneWay("Nvzn.isDemo"),isEnabledBinding:SC.Binding.oneWay("Nvzn.canApproveManager"),title:"Manager Login: Edit Time Cards",valueBinding:"Nvzn.canEditManager"}),employeeSiteCheck:SC.CheckboxView.design({layout:{top:310,left:40,width:300,height:20},isVisibleBinding:SC.Binding.oneWay("Nvzn.isDemo"),title:"Employee Login: Show Site Info inline",valueBinding:"Nvzn.showInlineSite"})})});
...
}}),customerAddress:SC.View.extend(SC.ContentDisplay,{classNames:"header-address".w(),isVisibleBinding:"Nvzn.isEmployee",layout:{top:60,left:0.11,right:200,height:31},contentBinding:"Nvzn.customerController.fullAddress",render:function(context){context.push("<span class='label'>Site Address:</span> ");
...
this.indicatorView.animate("left",newLeft,{duration:0.3,timing:"ease"})}}.observes("nowShowing")})}),footerView:SC.View.extend({classNames:["page-footer"],childViews:"submitButton".w(),layout:{bottom:0,right:0,width:320,height:44},saveButton:SC.ButtonView.extend({layout:{centerY:0,right:140,height:24,width:80},target:Nvzn.statechart,action:"saveRoster",title:"Save"}),submitButton:SC.ButtonView.extend({layout:{centerY:0,right:40,height:24,width:120},isVisibleBinding:SC.Binding.oneWay("Nvzn.canApproveManager"),target:Nvzn.statechart,action:"submit",title:"Submit Approvals"})})})}),sidebarView:SC.View.extend({classNames:"sidebar".w(),layout:{width:210},childViews:"logo calendarView jobsView".w(),logo:SC.View.extend({layout:{height:70},didCreateLayer:function(){var el=this.$()[0];
el.title=Nvzn.VERSION}}),calendarView:SC.View.extend({layout:{top:85,height:240},childViews:"headerView todayButtonView calendarView".w(),headerView:SC.View.extend({classNames:"sidebar-calendar-header".w(),layout:{height:16,left:13,width:80},render:function(context,firstTime){return context.addClass("sidebar-header").text("TIMESHEETS")
...
return"<div class='sc-view color' data-color='%@'>&nbsp;</div>".fmt(color)}},{title:"Employee",classNames:"name",key:"displayName",formatter:function(value){if(value.indexOf("ADHOCNOFILL")>=0){return"<span class='row-adhoc'>Shifts Not filled</span>"
}else{if(value.indexOf("ADHOC")>=0){return"<span class='row-adhoc'>Shifts to be filled</span>"
}else{if(value.indexOf("CANCELLED")>=0){return"<span class='row-cancelled'>Cancelled Shifts</span>"
}else{return value}}}}},{title:function(){return Nvzn.dateFromStart(0)}.property(),classNames:"day mon",key:1,formatter:Nvzn.TimeCard.fieldFormatter},{title:function(){return Nvzn.dateFromStart(1)
...
Nvzn.statechart.sendEvent("valueChanged",storeKey,[start,value])},didUpdateLayer:function(){arguments.callee.base.apply(this,arguments);
this.$(".row-adhoc").parents(".dashboard-row").css({"background-color":"lightblue"}).children(".dashboard-cell:not(.customer)").css("border-top","solid 1px grey");
this.$(".row-cancelled").parents(".dashboard-row").css({"background-color":"mistyrose"}).children(".dashboard-cell").css("border-top","solid 1px grey");
this.$("div.color").each(function(){var el=$(this),parent=el.parent(),color=el.data("color");
if(parent.hasClass("customer")){parent.siblings(".name").css("border-left-color",color)
}parent.css("background-color",color)})},click:function(evt){var target=evt.target;
...
if(!employeeId){return}}}),payslips:EO.TableView.design({columns:[{title:"Pay Day",key:"date",formatter:function(value){return value.toFormattedString("%D-%b-%Y")
}},{title:"Paid To",key:"paid_to",formatter:function(value){return value.toFormattedString("%D-%b-%Y")
}},{title:"Hours",key:"hours"},{title:"Taxable",key:"taxable"},{title:"Non-Tax",key:"non_tax"},{title:"Gross",key:"gross"},{title:"Tax",key:"tax"},{title:"Net",key:"net"},{title:"PaySlip",key:"url",formatter:function(value){return"<a href='%@'>Download</a>".fmt(value)
}}].map(function(hash){return EO.TableColumn.create(hash)}),contentBinding:"Nvzn.employeeController.payslips"})});

Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-5-g803ba3c-84",canEditManager:NO,canApproveManager:NO,showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create({hour:0,minute:0,second:0}),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")
...
}else{Nvzn.getPath("mainPage.tabView").set("items",[{title:"Coming Weeks",value:"sites_by_week"},{title:"OSH Report",value:"ohs_report"}])
...
}else{}}d.append(ret.join(""))}});Nvzn.WeeklyView=SC.View.extend(SC.ContentDisplay,{classNames:["weekly-view"],weeksToShow:2,displayProperties:["weeksToShow"],render:function(ctx,firstTime){var content=this.get("content")||[],weeks=this.get("weeksToShow"),date;

Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-27-ge597ead-106",isDemo:YES,canEditManager:NO,canApproveManager:NO,canApproveManagerDidChange:function(){if(!this.get("canApproveManager")){this.set("canEditManager",NO)
}}.observes("canApproveManager"),showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create({year:2013,month:4,day:4}),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")
...
desc=item.get("desc");if(item.get("type")!=="N"&&desc!==null){ret+="<span class='tc_desc' title='%@'>%@</span></br>".fmt(desc,desc)

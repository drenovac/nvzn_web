Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-9-gf9010a4-88",canEditManager:NO,canApproveManager:NO,showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create({hour:0,minute:0,second:0}),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")
...
content.forEach(function(i,idx){if(idx%7===0){ctx.push("</tr><tr>")}date=i.date;var dateStr=date.get("day");
if(date.get("month")!==currentMonth){dateStr=date.toFormattedString("%b")+" "+dateStr;
...
var addr=cust.get("fullAddress");context.begin("div").addClass("sidebar-jobs-item-number").attr("title",addr).text(id).end();

Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-17-gd14e3af-96",canEditManager:NO,canApproveManager:NO,showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create({hour:0,minute:0,second:0}),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")

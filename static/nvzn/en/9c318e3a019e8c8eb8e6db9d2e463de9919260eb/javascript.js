Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-21-g95b720e-100",isDemo:YES,canEditManager:NO,canApproveManager:NO,canApproveManagerDidChange:function(){if(!this.get("canApproveManager")){this.set("canEditManager",NO)
...
})}.property("timeCards","weekEnding").cacheable()});Nvzn.employeesController=SC.ArrayController.create({contentBinding:"Nvzn.customerController.employees",allowsEmptySelection:NO,orderBy:["customer","lastName"],arrangedObjects:function(){var customerId=null,customer,employees=this.get("content"),site,storeKey,S=Nvzn.store,C=Nvzn.Customer,ret=[];
...
Nvzn.getPath("mainPage.mainPane").append();if(Nvzn.loginController.get("role")=="site"){Nvzn.getPath("mainPage.tabView").set("items",[{title:"View Rosters",value:"all_employees"}])
}else{Nvzn.getPath("mainPage.tabView").set("items",[{title:"Roster",value:"sites_by_week"}])
}},tabDidChange:function(){switch(Nvzn.mainPage.getPath("tabView.nowShowing")){case"all_employees":this.gotoState("APP.ROSTER");
break;case"approve_tc":this.gotoState("APP.ROSTER");break;case"all_sites":this.gotoState("APP.ROSTER");
...
Nvzn.getPath("mainPage.header").set("isVisible",YES)},START_SITE:Ki.State.design({customerSelectionChanged:function(){console.log("first site selection changed...");
...
}},{title:"Employee",classNames:"name",key:"displayName",formatter:function(value){if(value.indexOf("ADHOC")>=0){return"<span class='row-adhoc'>Shifts to be filled</span>"

Nvzn=SC.Application.create({NAMESPACE:"Nvzn",VERSION:"201302041715-4-g859a52c-83",canEditManager:NO,canApproveManager:NO,showTimeCardColors:NO,store:SC.Store.create().from(SC.Record.fixtures),local:SC.UserDefaults.create({appDomain:"Nvzn"}),rosterContent:null,selectedRecord:null,selectedDate:SC.DateTime.create(),selectedWeekDidChange:function(){if(Nvzn.didChangeFor("core","selectedWeek")){Nvzn.statechart.sendEvent("selectedNewDay")
...
}.observes("selection")});Nvzn.employeeController=SC.ObjectController.create({weekEndingBinding:"Nvzn.weekEnding",weekEnding:null,customerTimecards:function(){var cardsBySite={},sunday,customer,date,ms,tcs=this.get("timeCards"),sites=SC.Set.create();
...
Nvzn.timeCardsByWeekController=SC.ArrayController.create({timeCardsBinding:"Nvzn.employeeController.timeCards",weeksToShow:4,startOfWeekBinding:"Nvzn.startOfWeek",timeCardsDidChange:function(){console.log("TimeCards or startOfWeek did change");
...
if(!timecards){return}for(i=0;i<daysToShow;i++){var day=startOfWeek.advance({day:i});
days.push({date:day,timecards:timecards.filter(function(timecard){return SC.DateTime.compareDate(day,timecard.get("dateObject"))===0
})})}console.log("TimeCards:");this.set("content",days)}.observes("timeCards","startOfWeek","weeksToShow")});
...
})},getEmployeeData:function(employee){var week=Nvzn.weeksFromWeekEnding(),url="/api/v1.1/employee/timecards"+(week?"?week="+week:""),weeksToShow=Nvzn.timeCardsByWeekController.get("weeksToShow");
...
if(!address){address={}}if(SC.empty(address.street)){if(SC.empty(address.name)){address.name="The Building."
}address.street="1 Some Place"}if(SC.empty(address.suburb)){address.suburb="That Town"
}return address.name+" "+address.street+", "+address.suburb+" "+address.postcode}.property("address").cacheable(),employees:SC.Record.toMany("Nvzn.Employee"),displayColor:null});
...
}if(color){cid=item.getPath("customer");ret+="<span class='customer-color' title='"+cid+"' style='border-left-color:"+Nvzn.colorFor(cid)+";'>"
...
Nvzn.getPath("mainPage.mainPane").append();if(Nvzn.loginController.get("role")=="site"){Nvzn.getPath("mainPage.tabView").set("items",[{title:"All Employees",value:"all_employees"}])
}else{Nvzn.getPath("mainPage.tabView").set("items",[{title:"This Week",value:"all_sites"},{title:"This Month",value:"sites_by_week"},{title:"OSH Report",value:"ohs_report"}])
}},tabDidChange:function(){switch(Nvzn.mainPage.getPath("tabView.nowShowing")){case"all_employees":this.gotoState("APP.ROSTER");
break;case"all_sites":this.gotoState("APP.ROSTER");break;case"ohs_report":this.gotoState("OHS_REPORT");
break}return YES},logout:function(){var url="/api/v1.1/logout";SC.Request.getUrl(url).notify(this,function(){Nvzn.statechart.sendEvent("didLogout")
...
Nvzn.mainPage.sites_by_week.displayDidChange()})}}),SHOWING_EMPLOYEE:Ki.State.design({selectedNewDay:function(){this.gotoState("LOADING_EMPLOYEE")
...
}}),customerAddress:SC.View.extend(SC.ContentDisplay,{classNames:"header-address".w(),layout:{top:55,left:496,right:200,height:31},contentBinding:"Nvzn.customerController.fullAddress",render:function(context){context.push("<span class='label'>Site Address:</span> ");
...
this.indicatorView.animate("left",newLeft,{duration:0.3,timing:"ease"})}}.observes("nowShowing")})}),footerView:SC.View.extend({childViews:"submitButton".w(),layout:{bottom:0,right:0,width:320,height:44},saveButton:SC.ButtonView.extend({layout:{centerY:0,right:140,height:24,width:80},target:Nvzn.statechart,action:"saveRoster",title:"Save"}),submitButton:SC.ButtonView.extend({layout:{centerY:0,right:40,height:24,width:120},target:Nvzn.statechart,action:"submit",title:"Submit Approvals"})})})}),sidebarView:SC.View.extend({classNames:"sidebar".w(),layout:{width:210},childViews:"logo calendarView jobsView".w(),logo:SC.View.extend({layout:{height:70},didCreateLayer:function(){var el=this.$()[0];
...
}}),listView:SC.ScrollView.extend({layout:{top:20,bottom:13,right:13},contentView:SC.ListView.extend({rowHeight:31,contentBinding:"Nvzn.customersController.arrangedObjects",selectionBinding:"Nvzn.customersController.selection",exampleView:SC.ListItemView.extend({displayProperties:"isSelected".w(),classNames:"sidebar-jobs-list-item",render:function(context){if(this.get("isSelected")){context.addClass("sel")
}context.begin("div").addClass("sidebar-jobs-item-number").text(this.getPath("content")).end().begin("span").addStyle("background-color",Nvzn.colorFor(this.get("content"))).end()
...
}}}}),sites_by_week:Nvzn.WeeklyView.design({contentBinding:"Nvzn.timeCardsByWeekController.arrangedObjects",weeksToShowBinding:"Nvzn.timeCardsByWeekController.weeksToShow"}),all_sites:EO.TableView.design({columns:[{title:"Site",classNames:"name",key:"id"},{title:"Mon",classNames:"day mon",key:1},{title:"Tue",classNames:"day tue",key:2},{title:"Wed",classNames:"day wed",key:3},{title:"Thu",classNames:"day thu",key:4},{title:"Fri",classNames:"day fri",key:5},{title:"Sat",classNames:"day sat",key:6},{title:"Sun",classNames:"day sun",key:0}].map(function(hash){return EO.TableColumn.create(hash)
